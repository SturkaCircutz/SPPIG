...
artifacts.
\input{cartpole_abstract_results.tex}
...
\begin{enumerate}
  ...
  \item \textbf{A compact programmatic controller.} We evaluate a two-mode
  constant-action diagnostic state machine with an explicit switch over pole
  angle and angular velocity, and separately track the current incomplete
  synthesis path in the audit.
...
The repository contains both a partial trace-based learner and a fixed
two-mode diagnostic controller in the CartPole grammar class.  The local result
table uses the fixed controller below, while current synthesized-student
metrics are tracked separately in the audit:
\input{cartpole_policy_fragment.tex}
...
\paragraph{Paper Figure 19 reference.}
For provenance, the repository also includes a manually transcribed CartPole
state machine from the rendered Figure 19 page of the original paper:
\input{cartpole_figure19_reference_fragment.tex}
This fragment is a reference policy for reevaluation and comparison; it is not
generated by the current synthesis implementation.
...
Right: the fixed diagnostic programmatic switch boundary in
$(\theta,\dot\theta)$ space.}
...
feed-forward PPO and fixed programmatic diagnostic policy both solve the
250-step training split.  On the full 15,000-step test split, the fixed
programmatic controller survives longer in mean reward, but still has zero
full-horizon success in the checked-in local diagnostic run.
...
300-second horizon.}
\label{tab:results}
\input{cartpole_results_table.tex}
\end{table}

The medium partial PPO sweep is reported separately from the fixed local
comparison, because it narrows the paper's compute budget while preserving both
PPO baselines, multiple seeds, sampled paper-range hyperparameters, and the full
300-second test horizon.
\input{cartpole_ppo_sweep_fragment.tex}
...
300-second generalization test.  The fixed diagnostic programmatic state
machine generalized better in survival time because its switching boundary
encodes a repeatable balancing rule rather than a short-horizon neural behavior.
...
and a compact fixed programmatic state machine under the SPPIG train/test split.
The result supports the qualitative claim that explicit programmatic structure
can improve long-horizon generalization, while also showing that careful
reinforcement-learning engineering and program-synthesis provenance are
necessary before such comparisons are credible.
