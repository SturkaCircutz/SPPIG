...
length 1.0.  In local diagnostics over 20 rollouts, feed-forward PPO and the
programmatic state machine both reach 100\% training success, but PPO obtains
0\% success on the full 300-second test horizon with mean test reward 910.6.
The programmatic state machine obtains 20\% full-horizon test success and mean
test reward 6275.4.  We also document three implementation bugs that materially
changed reinforcement-learning behavior: missing 5-second episode truncation,
...
\begin{table}[t]
\centering
\caption{Twenty-rollout local evaluation.  Test success uses the full
300-second horizon.}
\label{tab:results}
