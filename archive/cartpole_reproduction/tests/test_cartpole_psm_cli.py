...
class CartpolePSMCliTest(unittest.TestCase):
    ...
    def test_cli_records_parallel_trace_worker_status(self):
        ...
        self.assertEqual(status["effective_student_parallel_switch_candidate_workers_per_fit"], 1)
        self.assertEqual(status["effective_student_parallel_switch_candidate_slots"], 1)
        self.assertEqual(status["effective_total_student_switch_worker_slots"], 2)
        ...
        self.assertTrue(status["uses_parallel_student_transition_switch_optimization"])
        self.assertFalse(status["uses_parallel_student_switch_candidate_optimization"])
    ...
    def test_cli_writes_metrics_json(self):
        ...
        self.assertEqual(provenance["switch_timing"]["finite_difference_gradient_min_relative_improvement"], 1e-6)
        self.assertEqual(
            provenance["switch_timing"]["finite_difference_gradient_convergence"],
        ...
        for index, entry in enumerate(metrics["synthesis_history"], start=1):
            ...
            self.assertIn("trace_log_likelihood", entry["student_fit_history"][-1])
            self.assertIn("mean_trace_log_likelihood", entry["student_fit_history"][-1])
            self.assertTrue(math.isfinite(entry["student_fit_history"][-1]["trace_log_likelihood"]))
            self.assertTrue(math.isfinite(entry["student_fit_history"][-1]["mean_trace_log_likelihood"]))
